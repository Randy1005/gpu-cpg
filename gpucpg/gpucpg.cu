...
__global__ void update_successors_bu(
  ...
  if (tid < num_verts) {
    ...
    int successor_edges{0};
    for (int e = e_beg; e < e_end; e++) {
      const auto neighbor = oes[e];
      if (compact_deviation_counts != nullptr) {
        reachable_edges +=
          distances[tid] != INT_MAX && distances[neighbor] != INT_MAX;
      }
      const int wgt = owgts[e]*SCALE_UP;
    ...
    succs[tid] = succ;
    if (compact_deviation_counts != nullptr) {
      compact_deviation_counts[tid] = reachable_edges - successor_edges;
    }
  }
}
...
// Arena append variant for legacy GPG. It combines classification and fill,
// bounds-checks both append regions, and leaves the resulting logical sizes in
// the GPU-resident tail counters.
__global__ void expand_short_pile_arena(
  int* verts, int* edges, float* wgts, int* succs, int* dists,
  PfxtNode* short_pile, PfxtNode* long_pile, int window_start, int window_end,
  int* curr_tail_short, int* curr_tail_long, float split, float final_split,
  bool store_long, int short_capacity, int long_capacity, int* overflow) {
  const int node_idx = threadIdx.x + blockIdx.x * blockDim.x + window_start;
  if (node_idx >= window_end) return;
  auto v = short_pile[node_idx].to;
  const auto level = short_pile[node_idx].level;
  const auto slack = short_pile[node_idx].slack;
  while (v != -1) {
    for (auto eid = verts[v]; eid < verts[v + 1]; ++eid) {
      const auto neighbor = edges[eid];
      if (neighbor == succs[v] || dists[neighbor] == INT_MAX) continue;
      const auto new_slack = slack
        + static_cast<float>(dists[neighbor]) / SCALE_UP + wgts[eid]
        - static_cast<float>(dists[v]) / SCALE_UP;
      PfxtNode* output = nullptr;
      int output_idx = -1;
      int output_capacity = 0;
      if (new_slack <= split) {
        output_idx = atomicAdd(curr_tail_short, 1);
        output = short_pile;
        output_capacity = short_capacity;
      }
      else if (store_long && new_slack <= final_split) {
        output_idx = atomicAdd(curr_tail_long, 1);
        output = long_pile;
        output_capacity = long_capacity;
      }
      ...
      auto& new_path = output[output_idx];
      new_path.level = level + 1;
      new_path.from = v;
      new_path.to = neighbor;
      new_path.parent = node_idx;
...
__global__ void collect_tc_pfxt_active_sources_warp_aggregated(
  const int* current_v,
  const int n_active,
  int* source_epoch,
  const int epoch,
  ...
  const bool valid = tid < n_active && current_v[tid] != -1;
  const int v = valid ? current_v[tid] : -1;
  const unsigned int valid_mask = __ballot_sync(warp_mask, valid);

  bool new_source = false;
  if (valid) {
    const unsigned int peers = __match_any_sync(valid_mask, v);
    const int source_leader = __ffs(static_cast<int>(peers)) - 1;
    if (lane == source_leader) {
      new_source = atomicExch(source_epoch + v, epoch) != epoch;
    }
  }

  const unsigned int new_source_mask = __ballot_sync(warp_mask, new_source);
  if (new_source_mask == 0) {
    return;
  }
  const int append_leader = __ffs(static_cast<int>(new_source_mask)) - 1;
  int append_base = 0;
  if (lane == append_leader) {
    append_base = atomicAdd(active_source_count, __popc(new_source_mask));
  }
  append_base = __shfl_sync(warp_mask, append_base, append_leader);
  if (new_source) {
    const unsigned int lanes_before = lane == 0 ? 0u : ((1u << lane) - 1u);
    const int append_rank = __popc(new_source_mask & lanes_before);
    active_sources[append_base + append_rank] = v;
  }
}
...
__global__ void count_tc_pfxt_compact_groups_warp_aggregated(
  const int* current_v,
  const int n_active,
  const int* source_slots,
  int* compact_group_counts) {
  ...
  const int source_slot = source_slots[v];
  const unsigned int peers = __match_any_sync(__activemask(), source_slot);
  const int lane = static_cast<int>(threadIdx.x) & 31;
  if (lane == __ffs(static_cast<int>(peers)) - 1) {
    atomicAdd(
      &compact_group_counts[source_slot + 1],
      __popc(peers));
  }
}
...
__global__ void fill_tc_pfxt_compact_groups_warp_aggregated(
  const int* current_v,
  const int n_active,
  const int* source_slots,
  int* compact_group_cursor,
  ...
  int base = 0;
  if (lane == leader) {
    base = atomicAdd(&compact_group_cursor[source_slot], __popc(peers));
  }
  base = __shfl_sync(peers, base, leader);
  const unsigned int lanes_before = lane == 0 ? 0u : ((1u << lane) - 1u);
  const int peer_rank = __popc(peers & lanes_before);
  path_indices[base + peer_rank] = tid;
}
...
__global__ void deferred_lpq_tile_min_slacks(
  ...
  const float minimum = BlockReduce(storage).Reduce(local_min, cuda::minimum<>{});
...
__global__ void shadow_tc_pfxt_source_local_tile_resident_lpq(
  ...
  const float parent_min =
    FloatBlockReduce(float_reduce_storage).Reduce(local_parent_min, cuda::minimum<>{});
  __syncthreads();
  const float parent_max =
    FloatBlockReduce(float_reduce_storage).Reduce(local_parent_max, cuda::maximum<>{});
  __syncthreads();
  const float dev_min =
    FloatBlockReduce(float_reduce_storage).Reduce(local_dev_min, cuda::minimum<>{});
  __syncthreads();
  const float dev_max =
    FloatBlockReduce(float_reduce_storage).Reduce(local_dev_max, cuda::maximum<>{});
  __syncthreads();
  const float exact_min =
    FloatBlockReduce(float_reduce_storage).Reduce(local_exact_min, cuda::minimum<>{});
  __syncthreads();
  const float exact_max =
    FloatBlockReduce(float_reduce_storage).Reduce(local_exact_max, cuda::maximum<>{});
  __syncthreads();
  const float exact_long_min =
    FloatBlockReduce(float_reduce_storage).Reduce(local_exact_long_min, cuda::minimum<>{});
  __syncthreads();
...
__global__ void fill_tc_pfxt_source_local_parent_tile_bounds(
  ...
  __shared__ typename BlockReduce::TempStorage reduce_storage;
  const float block_min = BlockReduce(reduce_storage).Reduce(local_min, cuda::minimum<>{});
  __syncthreads();
  const float block_max = BlockReduce(reduce_storage).Reduce(local_max, cuda::maximum<>{});
  if (threadIdx.x == 0) {
...
__global__ void fill_tc_pfxt_source_local_dev_tile_bounds(
  ...
  const float block_min =
    FloatBlockReduce(float_reduce_storage).Reduce(local_min, cuda::minimum<>{});
  __syncthreads();
  const float block_max =
    FloatBlockReduce(float_reduce_storage).Reduce(local_max, cuda::maximum<>{});
  __syncthreads();
...
__global__ void count_tc_pfxt_source_local_tile_candidate_classes(
  ...
  const float* dev_deltas,
  const unsigned char* dev_reachable,
  const float* raw_edge_weights,
  const int* raw_succs,
  const int* raw_dists,
  const bool raw_fanout_deviations,
  const PfxtNode* short_pile,
  const int window_start,
  const float split,
  const float final_split,
  ...
  for (int product = threadIdx.x; product < n_products; product += blockDim.x) {
    ...
    const int parent_idx = window_start + active_idx;
    const auto& node = short_pile[parent_idx];
    if (raw_fanout_deviations) {
      const int edge = dev_base + local_dev;
      const int dst = dev_dsts[edge];
      if (dst != raw_succs[src]
          && raw_dists[src] != INT_MAX
          && raw_dists[dst] != INT_MAX) {
        const float delta =
          __fdiv_rn(static_cast<float>(raw_dists[dst]), float(SCALE_UP))
          + raw_edge_weights[edge]
          - __fdiv_rn(static_cast<float>(raw_dists[src]), float(SCALE_UP));
        candidate_class = tc_pfxt::classify_candidate(
          node.slack + delta, split, final_split,
...
__global__ void sample_tc_pfxt_defer_candidate_classes(
  const int* current_v,
  const int* path_indices,
  const int active_path_count,
  const int* dev_offsets,
  const float* dev_deltas,
  const unsigned char* dev_reachable,
  const PfxtNode* short_pile,
  ...
  if (sample >= sample_count || active_path_count <= 0) {
    return;
  }

  // Stratify over active paths, then sample one deviation. Weighting each
  // observation by its deviation degree estimates the product-weighted class
  // fractions without enumerating the parent x deviation product space.
  const unsigned long long numerator =
    (static_cast<unsigned long long>(sample) * 2ULL + 1ULL)
    * static_cast<unsigned long long>(active_path_count);
  const int active_rank = static_cast<int>(
    numerator / (2ULL * static_cast<unsigned long long>(sample_count)));
  const int active_idx = path_indices[active_rank];
  ...
  unsigned int hash = static_cast<unsigned int>(sample) ^ seed;
  hash ^= hash >> 16;
  hash *= 0x7feb352du;
  hash ^= hash >> 15;
  hash *= 0x846ca68bu;
  hash ^= hash >> 16;
  const int local_dev = static_cast<int>(hash % static_cast<unsigned int>(dev_count));
  ...
  const unsigned long long weight = static_cast<unsigned long long>(dev_count);
  atomicAdd(stats + 0, 1ULL);
  atomicAdd(stats + 1, weight);
  atomicAdd(stats + 2, candidate_class == tc_pfxt::CandidateClass::SHORT ? weight : 0ULL);
  atomicAdd(stats + 3, candidate_class == tc_pfxt::CandidateClass::LONG ? weight : 0ULL);
  atomicAdd(stats + 4, candidate_class == tc_pfxt::CandidateClass::SKIP ? weight : 0ULL);
  atomicAdd(stats + 5, weight * weight);
}
...
__global__ void mark_tc_pfxt_unsafe_ordinary_chains(
  const int* current_v,
  const int n_active,
  const unsigned int* chain_product_upper_bounds,
...
// full-successor-chain traversal.  Both kernels are device-gated so the
// policy remains resident on the GPU; scalar_state[0] is only a compact
// completion signal piggybacked with the output counts.
__global__ void count_tc_pfxt_adaptive_scalar_candidates(
  const int* current_v,
  const int n_active,
  const int* succs,
  const int* next_dev_vertex,
  ...
  const bool use_final_split,
  const bool skip_long_paths,
  const unsigned long long* adaptive_state,
  int* scalar_state) {
  __shared__ int block_short;
  __shared__ int block_long;
  if (threadIdx.x == 0) {
    block_short = 0;
    block_long = 0;
  }
  __syncthreads();
  if (!tc_pfxt::should_run_ordinary_branch(
        static_cast<tc_pfxt::AdaptiveMode>(adaptive_state[10]))) {
    return;
  }
  const int active_idx = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if (active_idx < n_active) {
    const float parent_slack = short_pile[window_start + active_idx].slack;
    int v = current_v[active_idx];
    while (v != -1) {
      for (int dev_idx = dev_offsets[v]; dev_idx < dev_offsets[v + 1]; ++dev_idx) {
        if (dev_reachable != nullptr && dev_reachable[dev_idx] == 0) {
          continue;
        }
        const auto candidate_class = tc_pfxt::classify_candidate(
          parent_slack + dev_deltas[dev_idx],
          split,
          final_split,
          use_final_split,
          skip_long_paths);
        if (candidate_class == tc_pfxt::CandidateClass::SHORT) {
          atomicAdd(&block_short, 1);
        }
        else if (candidate_class == tc_pfxt::CandidateClass::LONG) {
          atomicAdd(&block_long, 1);
        }
      }
  ...
  __syncthreads();
  if (threadIdx.x == 0) {
    atomicAdd(scalar_state + 1, block_short);
    atomicAdd(scalar_state + 2, block_long);
  }
}

__global__ void fill_tc_pfxt_adaptive_scalar_candidates(
  int* current_v,
  const int n_active,
  const int* succs,
  const int* next_dev_vertex,
  ...
  const int parent_idx = window_start + active_idx;
  const PfxtNode parent = short_pile[parent_idx];
  int v = current_v[active_idx];
  while (v != -1) {
    for (int dev_idx = dev_offsets[v]; dev_idx < dev_offsets[v + 1]; ++dev_idx) {
      if (dev_reachable != nullptr && dev_reachable[dev_idx] == 0) {
      ...
      const float new_slack = parent.slack + dev_deltas[dev_idx];
      const auto candidate_class = tc_pfxt::classify_candidate(
        new_slack,
        split,
        final_split,
      ...
      int capacity = 0;
      if (candidate_class == tc_pfxt::CandidateClass::SHORT) {
        output_idx = atomicAdd(tail_short, 1);
        output = short_pile;
        capacity = short_capacity;
      }
      else if (candidate_class == tc_pfxt::CandidateClass::LONG) {
        output_idx = atomicAdd(tail_long, 1);
        output = long_pile;
        capacity = long_capacity;
      }
      if (output_idx >= 0) {
        if (output_idx >= capacity || output == nullptr) {
          atomicExch(overflow, 1);
        }
        else {
          output[output_idx] = PfxtNode{
            parent.level + 1,
            v,
            dev_dsts[dev_idx],
            parent_idx,
  ...
  current_v[active_idx] = -1;
}

__global__ void update_tc_pfxt_defer_oracle_state(
  const TcPfxtSourceLocalStats* source_stats,
  const unsigned long long* sample_stats,
  const int low,
  const int high,
  const int skip_min_pct,
  const bool dynamic_policy,
  const int dynamic_long_credit_pct,
  unsigned long long* state,
  tc_pfxt::AdaptiveTelemetryEntry* telemetry,
  ...
  const tc_pfxt::AdaptivePolicyInput input{
    stats.active_paths,
    stats.parent_dev_products,
    sample_stats[1],
    sample_stats[4],
    sample_stats[2],
    sample_stats[3]};
  const auto policy = tc_pfxt::AdaptivePolicy{low, high, skip_min_pct};
  const auto recommendation = dynamic_policy
    ? tc_pfxt::choose_dynamic_adaptive_mode(
        input, policy, dynamic_long_credit_pct)
    : tc_pfxt::choose_adaptive_mode(input, policy);
  const auto previous = static_cast<tc_pfxt::AdaptiveMode>(state[10]);
  ...
  atomicAdd(state + 6, sample_stats[1]);
  atomicAdd(state + 7, sample_stats[2]);
  atomicAdd(state + 8, sample_stats[3]);
  atomicAdd(state + 9, sample_stats[4]);
  atomicExch(state + 10, static_cast<unsigned long long>(selected));
  if ((previous == tc_pfxt::AdaptiveMode::ORDINARY
       || previous == tc_pfxt::AdaptiveMode::DEFERRED)
      && previous != selected) {
    atomicAdd(state + 11, 1ULL);
  }
  if (telemetry != nullptr && telemetry_size != nullptr) {
    tc_pfxt::record_adaptive_telemetry(
      telemetry,
      telemetry_size,
      telemetry_capacity,
      {outer_step,
...
// Reused deferred decisions still count as executed adaptive steps, but avoid
// re-running the sampling oracle and its host synchronization. Products are
// deliberately zero here: only evaluated steps contribute oracle evidence.
__global__ void record_tc_pfxt_cached_adaptive_step(
  unsigned long long* state,
  tc_pfxt::AdaptiveTelemetryEntry* telemetry,
  unsigned int* telemetry_size,
  const unsigned int telemetry_capacity,
  const unsigned int outer_step,
  const unsigned int chain_substep,
  const unsigned long long active_paths) {
  if (blockIdx.x != 0 || threadIdx.x != 0) {
    return;
  }
  const auto selected = static_cast<tc_pfxt::AdaptiveMode>(state[10]);
  if (selected != tc_pfxt::AdaptiveMode::DEFERRED) {
    return;
  }
  atomicAdd(state + 0, 1ULL);
  atomicAdd(state + 1 + static_cast<unsigned long long>(selected), 1ULL);
  atomicAdd(state + 4, active_paths);
  tc_pfxt::record_adaptive_telemetry(
    telemetry,
    telemetry_size,
    telemetry_capacity,
    {outer_step, chain_substep, selected, active_paths, 0ULL});
}
...
__global__ void fill_tc_pfxt_source_local_tile_candidates(
  ...
  const unsigned long long* adaptive_state,
  unsigned long long* deferred_product_count) {
  const int tile_idx = blockIdx.x;
  if (tile_idx >= n_tiles) {
    return;
  }
  ...
  if (tile_class == tc_pfxt::CandidateTileClass::ALL_SHORT
    ...
    const auto selected_mode = adaptive_state == nullptr
      ? tc_pfxt::AdaptiveMode::UNINITIALIZED
      : static_cast<tc_pfxt::AdaptiveMode>(adaptive_state[10]);
    if (!emit_short && tc_pfxt::should_defer_all_long_tile(
          deferred_tiles != nullptr, adaptive_state != nullptr, selected_mode)) {
      ...
      if (threadIdx.x == 0) {
        ...
        if (deferred_product_count != nullptr) {
          atomicAdd(
    ...
    if (product < n_products) {
      ...
      if (fuse_tile_classification) {
        ...
        if (dst != raw_succs[src]
          ...
          const float delta =
            __fdiv_rn(static_cast<float>(raw_dists[dst]), float(SCALE_UP))
            + raw_edge_weights[dev_base + local_dev]
            - __fdiv_rn(static_cast<float>(raw_dists[src]), float(SCALE_UP));
          new_slack = node.slack + delta;
          candidate_class = tc_pfxt::classify_candidate(
            new_slack, split, final_split, use_final_split, skip_long_paths);
        }
      }
...
__global__ void fill_tc_pfxt_source_local_tile_short_candidates_direct(
  ...
  const float block_min_parent =
    FloatBlockReduce(float_reduce_storage).Reduce(
      local_min_parent_slack, cuda::minimum<>{});
  __syncthreads();
  const float block_max_parent =
    FloatBlockReduce(float_reduce_storage).Reduce(
      local_max_parent_slack, cuda::maximum<>{});
  __syncthreads();
  ...
  const float block_min_dev =
    FloatBlockReduce(float_reduce_storage).Reduce(
      local_min_dev_delta, cuda::minimum<>{});
  __syncthreads();
  const float block_max_dev =
    FloatBlockReduce(float_reduce_storage).Reduce(
      local_max_dev_delta, cuda::maximum<>{});
  __syncthreads();
  ...
  if (threadIdx.x == 0 && class_counts != nullptr) {
    atomicAdd(class_counts + 0, block_short);
    atomicAdd(class_counts + 2, block_skip);
  }
}
...
__global__ void count_tc_pfxt_compact_deviations(
  ...
  for (int src = blockIdx.x * blockDim.x + threadIdx.x;
       src < n_nodes; src += blockDim.x * gridDim.x) {
...
static int build_tc_pfxt_compact_deviation_offsets_on_device(
  const int n_nodes, const int* d_row_ptr, const int* d_col_idx,
  const int* d_succs, const int* d_dists,
  TcPfxtDeviceCompactStaticDeviationCsr& out,
  TcPfxtCompactBuildStageStats* stage_stats = nullptr,
  const bool counts_ready = false) {
  const auto offsets_begin = stage_stats == nullptr
    ? std::chrono::steady_clock::time_point{}
    : std::chrono::steady_clock::now();
  // Every slot is overwritten below. Avoid value-initializing this graph-sized
  // array; only the CSR sentinel at offsets[0] needs an explicit zero.
  out.offsets.resize(static_cast<std::size_t>(n_nodes) + 1, thrust::no_init);
  cudaMemsetAsync(thrust::raw_pointer_cast(out.offsets.data()), 0, sizeof(int));
  if (n_nodes <= 0) {
  ...
  // Write counts directly into offsets[1..N] and scan them in place. The old
  // path allocated and initialized a second N-element counts vector, then
  // copied its scan into offsets even though the counts were never reused.
  if (!counts_ready) {
    count_tc_pfxt_compact_deviations
      <<<std::max(1, std::min(4096, ROUNDUPBLOCKS(n_nodes, 256))), 256>>>(
        n_nodes, d_row_ptr, d_col_idx, d_succs, d_dists,
        thrust::raw_pointer_cast(out.offsets.data()) + 1);
    cudaCheckErrors("tc pfxt GPU compact deviation count failed");
  }
  thrust::inclusive_scan(
    out.offsets.begin() + 1, out.offsets.end(), out.offsets.begin() + 1);
  int total = 0;
...
static void build_tc_pfxt_compact_deviation_payload_on_device(
  const int n_nodes, const int total,
  const int* d_row_ptr, const int* d_col_idx, const float* d_weights,
  const int* d_succs, const int* d_dists,
  TcPfxtDeviceCompactStaticDeviationCsr& out,
  TcPfxtCompactBuildStageStats* stage_stats = nullptr) {
  const auto payload_begin = stage_stats == nullptr
    ? std::chrono::steady_clock::time_point{}
    : std::chrono::steady_clock::now();
  // The fill kernel writes exactly the compact range [0, total), so default
  // initialization is pure memory traffic on the cold setup path.
  out.dsts.resize(total, thrust::no_init);
  out.deltas.resize(total, thrust::no_init);
  if (total > 0) {
...
static int build_tc_pfxt_compact_deviations_on_device(
  ...
  TcPfxtDeviceCompactStaticDeviationCsr& out,
  TcPfxtCompactBuildStageStats* stage_stats = nullptr) {
  const int total = build_tc_pfxt_compact_deviation_offsets_on_device(
    n_nodes, d_row_ptr, d_col_idx, d_succs, d_dists, out, stage_stats);
  build_tc_pfxt_compact_deviation_payload_on_device(
    n_nodes, total, d_row_ptr, d_col_idx, d_weights, d_succs, d_dists,
    out, stage_stats);
  return total;
}
...
struct CpGen::TcPfxtStaticCache {
  ...
  thrust::device_vector<unsigned int> chain_product_upper_bounds;
  ...
  void clear_static_payload() {
    ...
    thrust::device_vector<unsigned int>().swap(
...
__global__ void compute_tc_pfxt_chain_product_upper_bounds(
  ...
  const auto bound = tc_pfxt::chain_product_upper_bound(
    start, dev_offsets, succs, next_dev_vertex);
  bounds[start] = static_cast<unsigned int>(
    min(bound, static_cast<unsigned long long>(UINT_MAX)));
}
...
struct TcPfxtScratch {
  ...
  thrust::device_vector<unsigned long long> deferred_lpq_product_count;
...
static TcPfxtPairDiscovery tc_pfxt_discover_pair_count_for_current_v(
  ...
  const bool scalar_discovery =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY") != nullptr;
  if (scalar_discovery) {
...
static void tc_pfxt_expand_window(
  ...
  const bool use_legacy_atomic =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_USE_LEGACY_ATOMIC") != nullptr;
  const bool shape_dispatch =
    false;
  const bool replay_discovery =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DISCOVERY_REPLAY") != nullptr;
  ...
  while (h_active > 0 && chain_substep < max_chain_substeps) {
    ...
    if (replay_discovery) {
      ...
      auto replay = [&](const bool scalar) {
        if (scalar) {
          setenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY", "1", 1);
        }
        else {
          unsetenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY");
        }
        Timer replay_timer;
        replay_timer.start();
        ...
        cudaCheckErrors("tc pfxt genuine replay fingerprint failed");
      ...
      (void)replay(false);
      (void)replay(true);
      const bool scalar_first = (substep_number & 1) != 0;
      ...
      std::cout << "adaptive_pfxt_discovery_replay"
        ...
        << '\n';
      if (!replay_match) {
        throw std::runtime_error("tc/scalar genuine discovery replay mismatch");
      }
      if (scalar_was_set) {
        setenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY", "1", 1);
      }
      ...
      (void)tc_pfxt_discover_pair_count_for_current_v(
        n_nodes,
        bvss,
        thrust::raw_pointer_cast(current_v.data()),
        n_active,
...
static void tc_pfxt_expand_window_single_pass(
  ...
  bool raw_source_local_ordinary,
  const unsigned int* chain_product_upper_bounds,
  ...
  std::uint64_t& total_pair_count,
  TcPfxtStepTiming& step_timing) {
  bool use_final_split = final_window_size > 0;
  const bool force_atomic_fallback =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_USE_ATOMIC_FALLBACK") != nullptr;
  const bool shape_dispatch =
    false;
  const bool use_family_queue_candidate =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_FAMILY_QUEUE_CANDIDATE") != nullptr;
  const bool use_compressed_lpq =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_COMPRESSED_LPQ") != nullptr;
  const bool use_threshold_filter =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_THRESHOLD_FILTER") != nullptr;
  const bool use_source_major_candidate =
    tc_pfxt::should_use_source_major_candidate_path(
      std::getenv("GPUCPG_ADAPTIVE_PFXT_SOURCE_MAJOR_CANDIDATE") != nullptr,
      std::getenv("GPUCPG_ADAPTIVE_PFXT_DISABLE_SOURCE_MAJOR_CANDIDATE") != nullptr,
  ...
  const bool use_single_work_candidate =
    tc_pfxt::should_use_single_work_candidate_path(
      std::getenv("GPUCPG_ADAPTIVE_PFXT_SINGLE_WORK_CANDIDATE") != nullptr,
      std::getenv("GPUCPG_ADAPTIVE_PFXT_DISABLE_SINGLE_WORK_CANDIDATE") != nullptr,
      use_family_queue_candidate || use_compressed_lpq || use_threshold_filter
        || use_source_major_candidate);
  const bool classify_experiment =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_CLASSIFY_EXPERIMENT") != nullptr;
  const bool classify_use =
    classify_experiment
    && std::getenv("GPUCPG_ADAPTIVE_PFXT_CLASSIFY_USE") != nullptr;
  const bool classify_validate =
    classify_experiment
    && std::getenv("GPUCPG_ADAPTIVE_PFXT_CLASSIFY_VALIDATE") != nullptr;
  const bool use_source_min_slack =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_ENABLE_SOURCE_MIN") != nullptr;
  const bool validate_source_min_slack =
    use_source_min_slack
    && std::getenv("GPUCPG_ADAPTIVE_PFXT_VALIDATE_SOURCE_MIN") != nullptr;
  const bool fused_interface_shadow =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_FUSED_INTERFACE_SHADOW") != nullptr;
  const bool fused_interface_shadow_strict =
    fused_interface_shadow
    && std::getenv("GPUCPG_ADAPTIVE_PFXT_FUSED_INTERFACE_SHADOW_STRICT") != nullptr;
  const bool in_discovery_short_only =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_IN_DISCOVERY_SHORT_ONLY") != nullptr;
  const bool direct_pair_meta =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DIRECT_PAIR_META") != nullptr;
  const int direct_pair_meta_capacity =
    get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_DIRECT_PAIR_META_CAP", 4194304);
  const bool use_aligned_bvss_consumer =
    false
    && aligned_bvss.n_vss > 0
    && !compact_static_devs.empty();
  const bool profile_source_local =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_SOURCE_LOCAL_PROFILE") != nullptr;
  const bool profile_tf32_mma_classification =
    false;
  const bool source_local_requested =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_SOURCE_LOCAL_CANDIDATE") != nullptr;
  const bool tile_native_candidate_requested =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_TILE_NATIVE_CANDIDATE") != nullptr;
  const bool compact_source_groups_requested =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_COMPACT_SOURCE_GROUPS") != nullptr;
  const bool tail_derived_class_counts_enabled =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DISABLE_TAIL_DERIVED_CLASS_COUNTS")
      == nullptr;
  const bool force_warp_aggregate_active_source_collection =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_WARP_AGGREGATE_ACTIVE_SOURCE_COLLECTION")
      != nullptr;
  const bool disable_warp_aggregate_active_source_collection =
    std::getenv(
      "GPUCPG_ADAPTIVE_PFXT_DISABLE_WARP_AGGREGATE_ACTIVE_SOURCE_COLLECTION")
      != nullptr;
  const int warp_aggregate_active_source_collection_min_active =
    get_env_int_or_default(
      "GPUCPG_ADAPTIVE_PFXT_WARP_AGGREGATE_ACTIVE_SOURCE_COLLECTION_MIN_ACTIVE",
      1048576);
  const bool force_warp_aggregate_compact_group_count =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_WARP_AGGREGATE_GROUP_COUNT") != nullptr;
  const bool disable_warp_aggregate_compact_group_count =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DISABLE_WARP_AGGREGATE_GROUP_COUNT")
      != nullptr;
  const int warp_aggregate_compact_group_count_min_active =
    get_env_int_or_default(
      "GPUCPG_ADAPTIVE_PFXT_WARP_AGGREGATE_GROUP_COUNT_MIN_ACTIVE",
      4096);
  const bool force_warp_aggregate_compact_group_fill =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_WARP_AGGREGATE_GROUP_FILL") != nullptr;
  const bool disable_warp_aggregate_compact_group_fill =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DISABLE_WARP_AGGREGATE_GROUP_FILL")
      != nullptr;
  const int warp_aggregate_compact_group_fill_min_active =
    get_env_int_or_default(
      "GPUCPG_ADAPTIVE_PFXT_WARP_AGGREGATE_GROUP_FILL_MIN_ACTIVE",
      4096);
  const int tile_native_min_products = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_TILE_NATIVE_MIN_PRODUCTS", 4096);
  const bool use_compact_static_devs =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_COMPACT_STATIC_DEVS") != nullptr
    && !compact_static_devs.empty();
  const bool adaptive_defer_requested =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_ADAPTIVE_DEFER") != nullptr;
  const bool has_source_local_devs =
    raw_source_local_ordinary || use_compact_static_devs || !static_devs.empty();
  const int single_work_max_slots =
    get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_SINGLE_WORK_MAX_SLOTS", 20000000);
  ...
  const bool candidate_fallback_needed =
    tc_pfxt::should_take_pre_oracle_fallback(
  ...
  const int chunked_max_long_pile =
    get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_CHUNKED_MAX_LONG_PILE", 50000000);
  ...
  const bool use_source_local_for_window =
    ...
    && !use_threshold_filter
	    ...
	    && std::getenv("GPUCPG_ADAPTIVE_PFXT_SOURCE_SELECTIVITY_PROFILE") == nullptr
	    && std::getenv("GPUCPG_ADAPTIVE_PFXT_FAMILY_CAPTURE_DIR") == nullptr;
  const bool use_compact_source_groups =
    ...
    && !use_source_min_slack
    && !classify_experiment
    && std::getenv("GPUCPG_ADAPTIVE_PFXT_TILE_RESIDENT_LPQ_SHADOW") == nullptr
    && std::getenv("GPUCPG_ADAPTIVE_PFXT_TILE_RESIDENT_LPQ_CHEAP_SHADOW") == nullptr
    && std::getenv("GPUCPG_ADAPTIVE_PFXT_TILE_FILTER_PROFILE") == nullptr
    && std::getenv("GPUCPG_ADAPTIVE_PFXT_SHORT_TILE_BOUNDS_O1") == nullptr;
  ...
  auto& deferred_lpq_product_count = scratch.deferred_lpq_product_count;
  auto& pair_count = scratch.pair_count;
  auto& overflow = scratch.overflow;
  ...
  const bool ordered_frontier_requested =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_ORDERED_FRONTIER") != nullptr;
  const unsigned long long ordered_frontier_min_products =
    static_cast<unsigned long long>(get_env_int_or_default(
      "GPUCPG_ADAPTIVE_PFXT_ORDERED_FRONTIER_MIN_PRODUCTS", 100000));

  const bool deferred_lpq_requested =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DEFERRED_TILE_LPQ") != nullptr;
  const bool defer_oracle_requested =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DEFER_ORACLE") != nullptr
    || adaptive_defer_requested;
  const bool defer_oracle_trace =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DEFER_ORACLE_TRACE") != nullptr;
  const int defer_oracle_low = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_DEFER_ORACLE_LOW", 60);
  const int defer_oracle_high = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_DEFER_ORACLE_HIGH", 70);
  const int defer_oracle_sample_count = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_DEFER_ORACLE_SAMPLES", 4096);
  const int defer_oracle_skip_min_pct = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_DEFER_ORACLE_SKIP_MIN_PCT", 50);
  const bool defer_oracle_dynamic_policy =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DYNAMIC_POLICY") != nullptr;
  const int defer_oracle_dynamic_long_credit_pct = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_DYNAMIC_LONG_CREDIT_PCT", 0);
  if (defer_oracle_requested
      && (defer_oracle_low < 0
          || defer_oracle_high < defer_oracle_low
          || defer_oracle_sample_count <= 0
          || defer_oracle_skip_min_pct < 0
          || defer_oracle_skip_min_pct > 100
          || defer_oracle_dynamic_long_credit_pct < 0
          || defer_oracle_dynamic_long_credit_pct > 100)) {
    throw std::runtime_error("invalid adaptive PFXT defer oracle bounds");
  }
  const bool source_local_tile_class_fastpath =
    !raw_source_local_ordinary
    && (std::getenv("GPUCPG_ADAPTIVE_PFXT_TILE_CLASS_FASTPATH") != nullptr
      || deferred_lpq_requested
      || adaptive_defer_requested);
  const bool source_local_tile_bound_fastpath =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_TILE_BOUND_FASTPATH") != nullptr
    || ordered_frontier_requested;
  const bool source_local_short_tile_bounds =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_SHORT_TILE_BOUNDS") != nullptr;
  const bool source_local_short_tile_bounds_o1 =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_SHORT_TILE_BOUNDS_O1") != nullptr;
  const bool tile_handoff_fusion_requested =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_TILE_HANDOFF_FUSION") != nullptr;
  const bool tile_resident_lpq_shadow_requested =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_TILE_RESIDENT_LPQ_SHADOW") != nullptr;
  const bool tile_resident_lpq_cheap_shadow_requested =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_TILE_RESIDENT_LPQ_CHEAP_SHADOW") != nullptr;
  if (raw_source_local_ordinary) {
    source_local_dev_offsets = d_fanout_adjp;
    source_local_dev_dsts = d_fanout_adjncy;
  }
  else if (use_compact_static_devs) {
  ...
  const bool profile_work_equivalence =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_WORK_EQUIV_PROFILE") != nullptr;
  const bool profile_cross_frontier_reuse =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_CROSS_FRONTIER_PROFILE") != nullptr;
  const bool profile_vss_descriptors =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_VSS_DESCRIPTOR_PROFILE") != nullptr;
  const bool replay_vss_descriptors =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_VSS_DESCRIPTOR_REPLAY") != nullptr;
  const bool replay_discovery =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DISCOVERY_REPLAY") != nullptr;
  const bool force_replay_discovery =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_FORCED_DISCOVERY_REPLAY") != nullptr;
  ...
  const char* work_equivalence_csv =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_WORK_EQUIV_CSV");
  const char* source_selectivity_csv =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_SOURCE_SELECTIVITY_PROFILE");
  ...
  std::uint64_t defer_oracle_gpg_products = 0;
  std::uint64_t defer_oracle_deferred_paths = 0;
  std::uint64_t defer_oracle_deferred_products = 0;
  std::uint64_t defer_oracle_calibrate_paths = 0;
  std::uint64_t defer_oracle_calibrate_products = 0;
  int defer_oracle_gpg_substeps = 0;
  int defer_oracle_deferred_substeps = 0;
  int defer_oracle_calibrate_substeps = 0;
  int defer_oracle_sampled_substeps = 0;
  double defer_oracle_estimated_short_products = 0.0;
  double defer_oracle_estimated_long_products = 0.0;
  double defer_oracle_estimated_skip_products = 0.0;
  std::uint64_t defer_oracle_exact_short_products = 0;
  std::uint64_t defer_oracle_exact_long_products = 0;
  std::uint64_t defer_oracle_exact_skip_products = 0;
  int defer_oracle_sample_gpg_substeps = 0;
  int defer_oracle_sample_deferred_substeps = 0;
  if (defer_oracle_requested) {
    defer_oracle_sample_stats.resize(6);
    thrust::fill(
    ...
    if (outer_step == 1 || defer_oracle_state.size() != 14) {
      ...
      defer_oracle_state[10] = static_cast<unsigned long long>(
        tc_pfxt::AdaptiveMode::UNINITIALIZED);
      const int telemetry_capacity = get_env_int_or_default(
        "GPUCPG_ADAPTIVE_PFXT_ADAPTIVE_TELEMETRY_CAP", 65536);
      defer_oracle_telemetry.resize(telemetry_capacity);
      defer_oracle_telemetry_size.resize(1);
      thrust::fill(
  ...
  const char* family_capture_dir_env =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_FAMILY_CAPTURE_DIR");
  ...
  const int family_capture_step = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_FAMILY_CAPTURE_STEP", 1);
  const int family_capture_substep = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_FAMILY_CAPTURE_SUBSTEP", 1);
  const bool family_capture_window =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_FAMILY_CAPTURE_WINDOW") != nullptr;

  const int adaptive_safe_ordinary_min_paths = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_ADAPTIVE_SAFE_MIN_PATHS", 65536);
  const int adaptive_fast_lane_stable_windows = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_ADAPTIVE_FAST_STABLE_WINDOWS", 2);
  const int adaptive_fast_lane_min_substeps = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_ADAPTIVE_FAST_MIN_SUBSTEPS", 4);
  const int adaptive_fast_lane_audit_interval = get_env_int_or_default(
    "GPUCPG_ADAPTIVE_PFXT_ADAPTIVE_FAST_AUDIT_WINDOWS", 16);
  const bool adaptive_fast_lane_audit =
    tc_pfxt::should_audit_adaptive_fast_lane(
      scratch.adaptive_fast_lane_windows_since_audit,
      adaptive_fast_lane_audit_interval);
  ...
  auto adaptive_window_mode = raw_source_local_ordinary
    ? tc_pfxt::AdaptiveMode::ORDINARY
    : (adaptive_fast_lane_this_window
      ? scratch.adaptive_fast_lane_mode
      : tc_pfxt::AdaptiveMode::UNINITIALIZED);
  bool adaptive_window_saw_ordinary = raw_source_local_ordinary;
  bool adaptive_window_saw_deferred = false;
  if (force_replay_discovery && n_active > 0) {
    const bool scalar_was_set =
      std::getenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY") != nullptr;
    auto replay = [&](const bool scalar) {
      if (scalar) setenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY", "1", 1);
      else unsetenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY");
      Timer timer;
      timer.start();
      const auto result = tc_pfxt_discover_pair_count_for_current_v(
        n_nodes, bvss, thrust::raw_pointer_cast(current_v.data()), n_active,
        frontier, active_vss, active_vss_size, pairs.data(), pairs.capacity(),
        pair_count, overflow, false, fixed_discover_blocks, shape_dispatch);
      cudaDeviceSynchronize();
      timer.stop();
      thrust::device_vector<unsigned long long> fingerprint(2, 0);
      tc_pfxt_pair_fingerprint
        <<<std::max(1, std::min(4096, ROUNDUPBLOCKS(result.n_pairs, 256))), 256>>>(
          pairs.data(), result.n_pairs,
          thrust::raw_pointer_cast(fingerprint.data()));
      cudaCheckErrors("forced discovery replay fingerprint failed");
      const thrust::host_vector<unsigned long long> host_fingerprint(fingerprint);
      return std::tuple{result.n_pairs, host_fingerprint[0], host_fingerprint[1],
                        timer.get_elapsed_time().count()};
    };
    (void)replay(false);
    (void)replay(true);
    const bool scalar_first = (outer_step & 1) != 0;
    const auto first = replay(scalar_first);
    const auto second = replay(!scalar_first);
    const auto& scalar_result = scalar_first ? first : second;
    const auto& tc_result = scalar_first ? second : first;
    const bool match = std::get<0>(scalar_result) == std::get<0>(tc_result)
      && std::get<1>(scalar_result) == std::get<1>(tc_result)
      && std::get<2>(scalar_result) == std::get<2>(tc_result);
    if (!match) {
      throw std::runtime_error("forced scalar/MMA discovery fingerprint mismatch");
    }
    if (scalar_was_set) setenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY", "1", 1);
    else unsetenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY");
    std::cout << "adaptive_pfxt_forced_window_discovery_replay"
      << " step=" << outer_step << " active_paths=" << n_active
      << " pairs=" << std::get<0>(tc_result)
      << " tc_us=" << std::get<3>(tc_result)
      << " scalar_us=" << std::get<3>(scalar_result)
      << " match=" << (match ? 1 : 0) << '\n';
  }
  while (h_active > 0 && chain_substep < max_chain_substeps) {
    const bool skip_long_this_substep = skip_long_paths || reached_k_after_window;
    const int substep_number = chain_substep + 1;
    if (false && force_replay_discovery && n_active > 0) {
      const bool scalar_was_set =
        std::getenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY") != nullptr;
      auto replay = [&](const bool scalar) {
        if (scalar) setenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY", "1", 1);
        ...
        const thrust::host_vector<unsigned long long> host_fingerprint(fingerprint);
        return std::tuple{result.n_pairs, host_fingerprint[0], host_fingerprint[1],
                          replay_timer.get_elapsed_time().count()};
      };
      (void)replay(false);
      (void)replay(true);
      ...
      std::cout << "adaptive_pfxt_forced_discovery_replay"
        << " step=" << outer_step << " substep=" << substep_number
        << " active_paths=" << n_active << " pairs=" << std::get<0>(tc_result)
        << " tc_us=" << std::get<3>(tc_result)
        << " scalar_us=" << std::get<3>(scalar_result)
        << " match=" << (match ? 1 : 0) << '\n';
    }
    bool adaptive_ordinary_this_substep = raw_source_local_ordinary;
    ...
    if (evaluate_adaptive_oracle) {
      ...
      const unsigned int sample_seed =
      ...
      update_tc_pfxt_defer_oracle_state<<<1, 1>>>(
        ...
        defer_oracle_low,
        defer_oracle_high,
        defer_oracle_skip_min_pct,
        defer_oracle_dynamic_policy,
        defer_oracle_dynamic_long_credit_pct,
    ...
    if (adaptive_defer_requested
      ...
      adaptive_window_mode = adaptive_ordinary_this_substep
        ? tc_pfxt::AdaptiveMode::ORDINARY
        : tc_pfxt::AdaptiveMode::DEFERRED;
      adaptive_window_saw_ordinary |= adaptive_ordinary_this_substep;
      adaptive_window_saw_deferred |= !adaptive_ordinary_this_substep;
      if (h_scalar_state[0] != 0) {
        int h_short_added = h_scalar_state[1];
        int h_long_added = h_scalar_state[2];
        const auto long_limit_bytes = static_cast<std::uint64_t>(
          get_env_int_or_default("GPUCPG_LONG_PILE_LIMIT_BYTES", 2000000000));
        const bool final_window_capacity_gate =
          tc_pfxt::should_prefetch_final_window(
              static_cast<std::uint64_t>(long_pile_size),
              static_cast<std::uint64_t>(h_long_added),
              sizeof(PfxtNode),
              long_limit_bytes,
              use_final_split,
              static_cast<std::uint64_t>(short_pile_size),
              static_cast<std::uint64_t>(h_short_added),
              static_cast<std::uint64_t>(k));
        if (defer_oracle_trace && final_window_capacity_gate) {
          std::cout << "adaptive_final_window_capacity_gate"
            << " outer_step=" << outer_step
            << " chain_substep=" << substep_number
            << " short_count=" << short_pile_size
            << " added_short=" << h_short_added
            << " long_count=" << long_pile_size
            << " added_long=" << h_long_added
            << " node_bytes=" << sizeof(PfxtNode)
            << " limit_bytes=" << long_limit_bytes
            << " deferred_backlog=" << scratch.deferred_lpq_remaining
            << " final_window_active=" << (use_final_split ? 1 : 0)
        ...
        if (scratch.deferred_lpq_remaining == 0
          ...
          if (num_short_paths_needed <= 0
              || num_short_paths_needed > long_pile_size) {
            throw std::runtime_error(
              "adaptive final-window capacity gate has invalid LPQ rank");
          }
          thrust::sort(long_pile.begin(), long_pile.end(), pfxt_node_comp());
          PfxtNode boundary;
          cudaMemcpy(
            &boundary,
            thrust::raw_pointer_cast(long_pile.data())
              + num_short_paths_needed - 1,
            sizeof(PfxtNode),
            cudaMemcpyDeviceToHost);
          cudaCheckErrors("adaptive final-window boundary copy failed");
          final_split = boundary.slack;
          final_window_size = static_cast<int>(thrust::remove_if(
            long_pile.begin(),
            long_pile.end(),
            [final_split] __host__ __device__ (const PfxtNode& node) {
              return node.slack > final_split;
            }) - long_pile.begin());
          ...
          count_tc_pfxt_adaptive_scalar_candidates
            <<<std::max(1, ROUNDUPBLOCKS(n_active, 256)), 256>>>(
              ...
              thrust::raw_pointer_cast(defer_oracle_state.data()),
              thrust::raw_pointer_cast(adaptive_scalar_state.data()));
          cudaCheckErrors("adaptive final-window recount failed");
          cudaMemcpy(
            h_scalar_state,
            thrust::raw_pointer_cast(adaptive_scalar_state.data()),
            sizeof(h_scalar_state),
          ...
          if (h_scalar_state[0] == 0) {
            throw std::runtime_error(
              "adaptive final-window recount lost ordinary mode");
          }
          h_short_added = h_scalar_state[1];
          h_long_added = h_scalar_state[2];
        }
        const bool substep_reaches_k =
          short_pile_size + h_short_added >= k;
        const bool fill_longs =
          !skip_long_this_substep && !substep_reaches_k;
        if (!fill_longs) {
          h_long_added = 0;
        }
        ...
        fill_tc_pfxt_adaptive_scalar_candidates
          <<<std::max(1, ROUNDUPBLOCKS(n_active, 256)), 256>>>(
            ...
            thrust::raw_pointer_cast(overflow.data()),
            thrust::raw_pointer_cast(defer_oracle_state.data()));
        cudaCheckErrors("tc pfxt adaptive scalar fill failed");

        // The count pass reserves exact, non-overlapping output ranges. Do not
        // synchronize merely to read tails back: their final values are the
        // capacities computed above, and subsequent stream work is ordered.
        short_pile_size = short_capacity;
    ...
    if (adaptive_defer_requested && !evaluate_adaptive_oracle) {
      ...
      record_tc_pfxt_cached_adaptive_step<<<1, 1>>>(
        ...
        static_cast<unsigned int>(substep_number),
        static_cast<unsigned long long>(h_active));
      cudaCheckErrors("tc pfxt cached adaptive step failed");
    }
    Timer phase_timer;
    phase_timer.start();
    auto light_stage_start = light_profiler.begin();
    ...
    const int* active_group_offsets =
      use_compact_source_groups
        ...
        : thrust::raw_pointer_cast(group_offsets.data());
    if (use_compact_source_groups) {
      const bool use_warp_aggregate_active_source_collection =
        tc_pfxt::should_warp_aggregate_group_count(
          n_active,
          warp_aggregate_active_source_collection_min_active,
          force_warp_aggregate_active_source_collection,
          disable_warp_aggregate_active_source_collection);
      if (use_warp_aggregate_active_source_collection) {
        collect_tc_pfxt_active_sources_warp_aggregated
          <<<std::max(1, ROUNDUPBLOCKS(n_active, 256)), 256>>>(
            thrust::raw_pointer_cast(current_v.data()),
            n_active,
            ...
            thrust::raw_pointer_cast(source_local_active_count.data()));
        cudaCheckErrors(
          "tc pfxt warp-aggregated compact collect active sources failed");
      }
      else {
      ...
      if (h_source_local_sources > 0) {
        ...
        const bool use_warp_aggregate_compact_group_count =
          tc_pfxt::should_warp_aggregate_group_count(
            n_active,
            warp_aggregate_compact_group_count_min_active,
            force_warp_aggregate_compact_group_count,
            disable_warp_aggregate_compact_group_count);
        const bool use_warp_aggregate_compact_group_fill =
          tc_pfxt::should_warp_aggregate_group_count(
            n_active,
            warp_aggregate_compact_group_fill_min_active,
            force_warp_aggregate_compact_group_fill,
            disable_warp_aggregate_compact_group_fill);
        if (use_warp_aggregate_compact_group_count) {
          count_tc_pfxt_compact_groups_warp_aggregated
            <<<std::max(1, ROUNDUPBLOCKS(n_active, 256)), 256>>>(
              thrust::raw_pointer_cast(current_v.data()),
              n_active,
              thrust::raw_pointer_cast(source_local_slots.data()),
              thrust::raw_pointer_cast(source_local_group_counts.data()));
          cudaCheckErrors("tc pfxt warp-aggregated compact count groups failed");
        }
        else {
        ...
        if (use_warp_aggregate_compact_group_fill) {
          fill_tc_pfxt_compact_groups_warp_aggregated
            <<<std::max(1, ROUNDUPBLOCKS(n_active, 256)), 256>>>(
              thrust::raw_pointer_cast(current_v.data()),
              n_active,
              thrust::raw_pointer_cast(source_local_slots.data()),
              thrust::raw_pointer_cast(source_local_group_cursor.data()),
              thrust::raw_pointer_cast(path_indices.data()));
          cudaCheckErrors("tc pfxt warp-aggregated compact fill groups failed");
        }
        else {
    ...
    bool defer_oracle_sampled = false;
    unsigned long long h_defer_oracle_sample_stats[6] {};
    if (emit_source_local_sources && h_source_local_sources > 0) {
      ...
      if (defer_oracle_requested
          && !adaptive_defer_requested
          && source_stats.active_paths > 0) {
        thrust::fill(
          defer_oracle_sample_stats.begin(),
          defer_oracle_sample_stats.end(),
          0ULL);
        const double product_intensity =
          static_cast<double>(source_stats.parent_dev_products)
          / static_cast<double>(source_stats.active_paths);
        const char* recommendation = "calibrate";
        defer_oracle_paths += source_stats.active_paths;
        defer_oracle_products += source_stats.parent_dev_products;
        if (product_intensity < static_cast<double>(defer_oracle_low)) {
          recommendation = "gpg";
          ++defer_oracle_gpg_substeps;
          defer_oracle_gpg_paths += source_stats.active_paths;
          defer_oracle_gpg_products += source_stats.parent_dev_products;
        }
        else if (product_intensity > static_cast<double>(defer_oracle_high)) {
          recommendation = "deferred";
          ++defer_oracle_deferred_substeps;
          defer_oracle_deferred_paths += source_stats.active_paths;
          defer_oracle_deferred_products += source_stats.parent_dev_products;
        }
        else {
          ++defer_oracle_calibrate_substeps;
          defer_oracle_calibrate_paths += source_stats.active_paths;
          defer_oracle_calibrate_products += source_stats.parent_dev_products;
          const int active_path_count = static_cast<int>(source_stats.active_paths);
          const int samples = std::min(
            defer_oracle_sample_count,
            active_path_count);
          const unsigned int sample_seed =
            static_cast<unsigned int>(outer_step) * 0x9e3779b9u
            ^ static_cast<unsigned int>(substep_number) * 0x85ebca6bu
            ^ static_cast<unsigned int>(window_start);
          sample_tc_pfxt_defer_candidate_classes
            <<<std::max(1, ROUNDUPBLOCKS(samples, 256)), 256>>>(
              thrust::raw_pointer_cast(current_v.data()),
              thrust::raw_pointer_cast(path_indices.data()),
              active_path_count,
              source_local_dev_offsets,
              ...
              sample_seed,
              thrust::raw_pointer_cast(defer_oracle_sample_stats.data()));
          cudaCheckErrors("tc pfxt defer oracle sample failed");
          if (defer_oracle_trace) {
            cudaMemcpy(
              h_defer_oracle_sample_stats,
              thrust::raw_pointer_cast(defer_oracle_sample_stats.data()),
              sizeof(h_defer_oracle_sample_stats),
              cudaMemcpyDeviceToHost);
            cudaCheckErrors("tc pfxt defer oracle sample copy failed");
            defer_oracle_sampled = h_defer_oracle_sample_stats[1] > 0;
          }
        }
        update_tc_pfxt_defer_oracle_state<<<1, 1>>>(
          thrust::raw_pointer_cast(source_local_stats.data()),
        ...
        if (defer_oracle_trace) {
          std::cout << "adaptive_pfxt_defer_oracle"
            << " step=" << outer_step
            << " substep=" << substep_number
            << " active_paths=" << source_stats.active_paths
            << " products=" << source_stats.parent_dev_products
            << " products_per_path=" << product_intensity
            << " low=" << defer_oracle_low
            << " high=" << defer_oracle_high
      ...
      if (profile_cross_frontier_reuse) {
        ...
        if (histogram_pairs != static_cast<unsigned long long>(
              profile_discovery.n_pairs)) {
          std::cerr << "adaptive_pfxt_cross_frontier_mismatch"
            << " step=" << outer_step
            << " substep=" << substep_number
      ...
      direct_pair_meta
        ...
        if (replay_discovery) {
          ...
          auto replay = [&](const bool scalar) {
            ...
            tc_pfxt_pair_fingerprint
              <<<std::max(1, std::min(
                   4096, ROUNDUPBLOCKS(result.n_pairs, 256))), 256>>>(
                pairs.data(),
                result.n_pairs,
                thrust::raw_pointer_cast(fingerprint.data()));
            cudaCheckErrors("tc pfxt generic replay fingerprint failed");
            thrust::host_vector<unsigned long long> host_fingerprint(fingerprint);
            return std::tuple{
              result.n_pairs,
              host_fingerprint[0],
          ...
          const bool scalar_first =
            ((outer_step + substep_number) & 1) != 0;
          const auto first_replay = replay(scalar_first);
          const auto second_replay = replay(!scalar_first);
          const auto& tc_replay =
            scalar_first ? second_replay : first_replay;
          const auto& scalar_replay =
            scalar_first ? first_replay : second_replay;
          const bool replay_match =
            std::get<0>(tc_replay) == std::get<0>(scalar_replay)
            && std::get<1>(tc_replay) == std::get<1>(scalar_replay)
            && std::get<2>(tc_replay) == std::get<2>(scalar_replay);
          ...
          if (!replay_match) {
            throw std::runtime_error(
              "tc/scalar generic discovery replay mismatch");
          }
          if (scalar_was_set) {
            setenv("GPUCPG_ADAPTIVE_PFXT_SCALAR_DISCOVERY", "1", 1);
          }
      ...
      if (capture_this_substep) {
        ...
        const bool compare_source_local_capture =
          std::getenv("GPUCPG_ADAPTIVE_PFXT_CAPTURE_COMPARE_SOURCE_LOCAL") != nullptr;
        ...
        if (compare_source_local_capture && !h_static_offsets.empty()) {
          ...
          std::cout << "adaptive_pfxt_capture_source_local_compare"
            << " step=" << outer_step
            << " substep=" << substep_number
        ...
        std::cout << "adaptive_pfxt_family_capture=" << filename
      ...
      if (handled_source_local_substep) {
        ...
        const int source_local_parent_tile =
          get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_SOURCE_LOCAL_PARENT_TILE", 32);
        const int source_local_dev_tile =
          get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_SOURCE_LOCAL_DEV_TILE", 16);
        ...
        unsigned long long h_source_local_class_counts_raw[4] =
          {0ULL, 0ULL, 0ULL, 0ULL};
        auto record_defer_oracle_sample = [&]() {
          if (!defer_oracle_sampled) {
            return;
          }
          const double sample_weight =
            static_cast<double>(h_defer_oracle_sample_stats[1]);
          const double sample_short =
            static_cast<double>(h_defer_oracle_sample_stats[2]) / sample_weight;
          const double sample_long =
            static_cast<double>(h_defer_oracle_sample_stats[3]) / sample_weight;
          const double sample_skip =
            static_cast<double>(h_defer_oracle_sample_stats[4]) / sample_weight;
          const double sum_weight_squared =
            static_cast<double>(h_defer_oracle_sample_stats[5]);
          const double effective_samples = sum_weight_squared == 0.0
            ? 0.0
            : sample_weight * sample_weight / sum_weight_squared;
          const double skip_standard_error = effective_samples <= 0.0
            ? 0.0
            : std::sqrt(sample_skip * (1.0 - sample_skip) / effective_samples);
          const bool sample_recommends_deferred =
            sample_skip * 100.0
            >= static_cast<double>(defer_oracle_skip_min_pct);
          ++defer_oracle_sampled_substeps;
          defer_oracle_estimated_short_products +=
            sample_short * static_cast<double>(source_local_products);
          defer_oracle_estimated_long_products +=
            sample_long * static_cast<double>(source_local_products);
          defer_oracle_estimated_skip_products +=
            sample_skip * static_cast<double>(source_local_products);
          defer_oracle_exact_short_products += h_source_local_class_counts_raw[0];
          defer_oracle_exact_long_products += h_source_local_class_counts_raw[1];
          defer_oracle_exact_skip_products += h_source_local_class_counts_raw[2];
          if (sample_recommends_deferred) {
            ++defer_oracle_sample_deferred_substeps;
          }
          else {
            ++defer_oracle_sample_gpg_substeps;
          }
          std::cout << "adaptive_pfxt_defer_oracle_sample"
            << " step=" << outer_step
            << " substep=" << substep_number
            << " samples=" << h_defer_oracle_sample_stats[0]
            << " effective_samples=" << effective_samples
            << " estimated_short_fraction=" << sample_short
            << " estimated_long_fraction=" << sample_long
            << " estimated_skip_fraction=" << sample_skip
            << " estimated_skip_ci95_low="
            << std::max(0.0, sample_skip - 1.96 * skip_standard_error)
            << " estimated_skip_ci95_high="
            << std::min(1.0, sample_skip + 1.96 * skip_standard_error)
            << " exact_short_fraction="
            << static_cast<double>(h_source_local_class_counts_raw[0])
              / static_cast<double>(source_local_products)
            << " exact_long_fraction="
            << static_cast<double>(h_source_local_class_counts_raw[1])
              / static_cast<double>(source_local_products)
            << " exact_skip_fraction="
            << static_cast<double>(h_source_local_class_counts_raw[2])
              / static_cast<double>(source_local_products)
            << " recommendation="
            << (sample_recommends_deferred ? "deferred" : "gpg")
            << '\n';
        };
        ...
        const bool use_deferred_lpq =
          ...
          && std::getenv("GPUCPG_ADAPTIVE_PFXT_CANDIDATE_ARENA_SHADOW_COUNT")
            != nullptr;
        ...
        if (h_n_tiles > 0) {
          ...
          if (profile_tf32_mma_classification) {
            ...
            std::cout << "adaptive_pfxt_tf32_mma_classification"
              << " tiles=" << h_n_tiles
          ...
          if (profile_source_local_tile_filter) {
            ...
            std::cout << "adaptive_pfxt_tf32_score_oracle"
            ...
            for (int bin = 0; bin < 5; ++bin) {
              ...
              std::cout << "adaptive_pfxt_frontier_oracle"
          ...
          if (!use_tile_native_short_only && use_tile_bound_fastpath) {
            ...
            count_tc_pfxt_source_local_tile_candidate_classes
              <<<h_n_tiles, TC_PFXT_PAIR_BLOCK_THREADS>>>(
                ...
                use_compact_source_groups,
                thrust::raw_pointer_cast(source_local_class_counts.data()),
                !use_tile_native_short_only && source_local_tile_class_fastpath
                  ? thrust::raw_pointer_cast(source_local_tile_classes.data())
                  : nullptr);
        ...
        const auto allocation_counts = tc_pfxt::source_local_allocation_counts(
          ...
          h_source_local_class_counts_raw[1]);
        // Both consumers emit one node per counted SHORT output.  In
        ...
        auto resize_detail_start = light_profiler.begin();
        const auto exact_short_limit = tc_pfxt::exact_short_output_limit(
          static_cast<std::uint64_t>(base_short),
          static_cast<std::uint64_t>(short_added_capacity));
        if (exact_short_limit
            > static_cast<std::uint64_t>(std::numeric_limits<int>::max())) {
          throw std::runtime_error(
            "tc pfxt exact short output limit exceeds index range");
        }
        int short_limit = static_cast<int>(exact_short_limit);
        short_pile.resize(short_limit);
        ...
        if (tc_pfxt::should_precount_short_outputs(use_tile_native_short_only)) {
          // The fill kernels own the production telemetry.  Do not carry the
          // sizing pass counts into that telemetry.
          thrust::fill(
            source_local_class_counts.begin(),
            source_local_class_counts.end(),
            0ULL);
        ...
        if (use_deferred_lpq) {
          ...
          deferred_lpq_product_count.resize(1);
          thrust::fill(
            deferred_lpq_product_count.begin(),
            deferred_lpq_product_count.end(),
            0ULL);
        }
        reset_tc_pfxt_candidate_state<<<1, 1>>>(
          d_tail_short,
        ...
        const bool use_tail_derived_class_counts =
          tail_derived_class_counts_enabled
          && use_tile_native_short_only
          && !use_aligned_bvss_consumer;
        auto* tile_native_class_counts = use_tail_derived_class_counts
          ? nullptr
          : thrust::raw_pointer_cast(source_local_class_counts.data());
        if (h_n_tiles > 0) {
          auto fill_detail_start = light_profiler.begin();
          if (use_tile_native_short_only) {
            const bool collect_bound_stats =
              source_local_short_tile_bounds
              && std::getenv("GPUCPG_ADAPTIVE_PFXT_SHORT_TILE_BOUNDS_STATS") != nullptr
              && !use_tile_handoff_fusion;
              ...
              if (use_aligned_short_fill) {
                ...
                fill_tc_pfxt_source_local_tile_short_candidates_direct
                  <<<h_n_tiles, TC_PFXT_PAIR_BLOCK_THREADS>>>(
                    ...
                    use_final_split,
                    source_local_short_tile_bounds
                      && !source_local_short_tile_bounds_o1
                      && !use_tile_handoff_fusion,
            ...
            if (h_tile_native_overflow != 0) {
              ...
              launch_tile_native_fill();
              std::cout << "adaptive_pfxt_short_capacity_retry"
            ...
            fill_tc_pfxt_source_local_tile_candidates
              <<<h_n_tiles, TC_PFXT_PAIR_BLOCK_THREADS>>>(
                ...
                adaptive_defer_requested
                  ...
                  : nullptr,
                use_deferred_lpq
                  ? thrust::raw_pointer_cast(deferred_lpq_product_count.data())
                  : nullptr);
        ...
        unsigned long long h_deferred_products = 0;
        if (use_deferred_lpq) {
          cudaMemcpy(
        ...
        const int substep_long = fill_longs
          ? h_tail_long - base_long + static_cast<int>(h_deferred_products)
          : 0;
        if (use_tile_native_short_only) {
          if (use_tail_derived_class_counts) {
            const auto total_products =
              static_cast<std::uint64_t>(source_local_products);
            const auto emitted_short =
              static_cast<std::uint64_t>(std::max(0, substep_short));
            if (!tc_pfxt::valid_short_only_class_counts(
                  total_products, emitted_short)) {
              throw std::runtime_error(
                "tc pfxt tail-derived short count exceeds total products");
            }
            const auto derived_counts =
              tc_pfxt::derive_short_only_class_counts(
                total_products, emitted_short);
            h_source_local_class_counts_raw[0] = derived_counts.short_count;
            h_source_local_class_counts_raw[1] = 0ULL;
            h_source_local_class_counts_raw[2] = derived_counts.skip_count;
            h_source_local_class_counts_raw[3] = 0ULL;
          }
          else {
            cudaMemcpy(
              h_source_local_class_counts_raw,
              ...
              cudaMemcpyDeviceToHost);
            cudaCheckErrors(
              "tc pfxt source-local copy tile-native class outputs failed");
          }
        ...
        const int source_major_parent_tile =
          get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_SOURCE_MAJOR_PARENT_TILE", 32);
        const int source_major_family_tile =
          get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_SOURCE_MAJOR_FAMILY_TILE", 16);
        ...
        thrust::host_vector<int> h_overflow(overflow);
        if (h_overflow[0] != 0) {
          std::cerr << "adaptive_pfxt_single_work_overflow_detail"
        ...
        const int chunk_limit =
          get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_CHUNK_PAIRS", 1048576);
        int substep_short = 0;
        int substep_long = 0;
  ...
  if (replay_vss_descriptors) {
    std::cout << "adaptive_pfxt_vss_descriptor_replay"
      << " step=" << outer_step
  ...
  if (profile_vss_descriptors) {
    ...
    if (h_stats[2] != vss_descriptor_expected_pairs) {
      std::cerr << "adaptive_pfxt_vss_descriptor_mismatch"
        << " step=" << outer_step
    ...
    std::cout << "adaptive_pfxt_vss_descriptor_profile"
      << " step=" << outer_step
  ...
  if (profile_cross_frontier_reuse) {
    ...
    std::cout << "adaptive_pfxt_cross_frontier_profile"
      << " step=" << outer_step
  ...
  if (defer_oracle_trace) {
    const thrust::host_vector<unsigned long long> h_defer_oracle_state(
      defer_oracle_state);
    std::cout << "adaptive_pfxt_defer_oracle_device_state"
      << " step=" << outer_step
      << " substeps=" << h_defer_oracle_state[0]
      << " gpg_substeps=" << h_defer_oracle_state[1]
      << " deferred_substeps=" << h_defer_oracle_state[2]
      << " unresolved_substeps=" << h_defer_oracle_state[3]
      << " active_paths=" << h_defer_oracle_state[4]
      << " products=" << h_defer_oracle_state[5]
      << " sample_weight=" << h_defer_oracle_state[6]
      << " sample_short_weight=" << h_defer_oracle_state[7]
      << " sample_long_weight=" << h_defer_oracle_state[8]
      << " sample_skip_weight=" << h_defer_oracle_state[9]
      << " cached_recommendation=" << h_defer_oracle_state[10]
      << " switches=" << h_defer_oracle_state[11]
      << '\n';
    const double aggregate_product_intensity = defer_oracle_paths == 0
      ? 0.0
      : static_cast<double>(defer_oracle_products)
        / static_cast<double>(defer_oracle_paths);
    std::cout << "adaptive_pfxt_defer_oracle_summary"
      << " step=" << outer_step
      << " low=" << defer_oracle_low
      << " high=" << defer_oracle_high
      << " active_paths=" << defer_oracle_paths
      << " products=" << defer_oracle_products
      << " products_per_path=" << aggregate_product_intensity
      << " gpg_substeps=" << defer_oracle_gpg_substeps
      << " gpg_paths=" << defer_oracle_gpg_paths
      << " gpg_products=" << defer_oracle_gpg_products
      << " deferred_substeps=" << defer_oracle_deferred_substeps
      << " deferred_paths=" << defer_oracle_deferred_paths
      << " deferred_products=" << defer_oracle_deferred_products
      << " calibrate_substeps=" << defer_oracle_calibrate_substeps
      << " calibrate_paths=" << defer_oracle_calibrate_paths
      << " calibrate_products=" << defer_oracle_calibrate_products
      << '\n';
    if (defer_oracle_sampled_substeps > 0) {
      const double sampled_products = static_cast<double>(
        defer_oracle_exact_short_products
        + defer_oracle_exact_long_products
        + defer_oracle_exact_skip_products);
      std::cout << "adaptive_pfxt_defer_oracle_sample_summary"
        << " step=" << outer_step
        << " sampled_substeps=" << defer_oracle_sampled_substeps
        << " estimated_short_fraction="
        << defer_oracle_estimated_short_products / sampled_products
        << " estimated_long_fraction="
        << defer_oracle_estimated_long_products / sampled_products
        << " estimated_skip_fraction="
        << defer_oracle_estimated_skip_products / sampled_products
        << " exact_short_fraction="
        << static_cast<double>(defer_oracle_exact_short_products)
          / sampled_products
        << " exact_long_fraction="
        << static_cast<double>(defer_oracle_exact_long_products)
          / sampled_products
        << " exact_skip_fraction="
        << static_cast<double>(defer_oracle_exact_skip_products)
          / sampled_products
        << " gpg_substeps=" << defer_oracle_sample_gpg_substeps
        << " deferred_substeps=" << defer_oracle_sample_deferred_substeps
  ...
  if (chunked_candidate_substeps > 0) {
    std::cout << "adaptive_pfxt_chunked_candidate_substeps="
      << chunked_candidate_substeps
  ...
  if (use_threshold_filter && threshold_total_possible > 0) {
    ...
    std::cout << "adaptive_pfxt_threshold_filter"
  ...
  if (classify_experiment) {
    std::cout << "adaptive_pfxt_classify_validation_substeps="
      << classify_validation_substeps
...
void CpGen::report_paths(
  ...
  if (tc_pfxt_use_cached_sfxt) {
    ...
    std::cout << "adaptive_pfxt_static_cache=hit sfxt=1\n";
    h_dists.assign(
    ...
    if (tc_pfxt_static_cache_supported && _tc_pfxt_static_cache->enabled) {
      ...
      std::cout << "adaptive_pfxt_static_cache=miss sfxt=0\n";
    }
	timer_cpg.start();
  ...
  std::cout << "sfxt_propagation time_ms=" << prop_time / 1ms << '\n';

  const auto sfxt_postprocess_begin = std::chrono::steady_clock::now();
  ...
  const auto tc_pfxt_successor_finalize_begin = std::chrono::steady_clock::now();
  ...
  const double tc_pfxt_successor_finalize_ms =
    std::chrono::duration<double, std::milli>(
      std::chrono::steady_clock::now() - tc_pfxt_successor_finalize_begin).count();
  std::cout << "sfxt_successor_finalize"
    << " fused_counts=0"
    << " time_ms=" << tc_pfxt_successor_finalize_ms << '\n';
  const double sfxt_postprocess_ms =
    std::chrono::duration<double, std::milli>(
      std::chrono::steady_clock::now() - sfxt_postprocess_begin).count();
  std::cout << "sfxt_postprocess"
    << " fused_counts=0"
    << " time_ms=" << sfxt_postprocess_ms << '\n';
  if (tc_pfxt_static_cache_supported && _tc_pfxt_static_cache->enabled) {
  ...
  const bool enable_tc_pfxt = std::getenv("GPUCPG_ENABLE_ADAPTIVE_PFXT") != nullptr;
  const int tc_pfxt_min_short_capacity =
    get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_MIN_SHORT_CAPACITY", 2500000);
  constexpr int default_tc_pfxt_max_pairs = 369386496;
  const int tc_pfxt_max_pairs =
    get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_MAX_PAIRS", default_tc_pfxt_max_pairs);
  const bool enable_tc_pfxt_fusion = std::getenv("GPUCPG_ADAPTIVE_PFXT_FUSION") != nullptr;
  const bool enable_tc_pfxt_single_pass =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_SINGLE_PASS") != nullptr;
  ...
  const int tc_pfxt_single_pass_fallback_long_pile =
    get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_SINGLE_PASS_FALLBACK_LONG_PILE", 1000000);
  const bool disable_tc_pfxt_phase_profile =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DISABLE_PHASE_PROFILE") != nullptr;
  const bool profile_tc_pfxt_phases = enable_interm_perf_log
    && !disable_tc_pfxt_phase_profile
    && (!enable_tc_pfxt_fusion || std::getenv("GPUCPG_ADAPTIVE_PFXT_PROFILE_PHASES") != nullptr);
  const bool light_tc_pfxt_stage_profile =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_LIGHT_STAGE_PROFILE") != nullptr;
  const int tc_pfxt_active_check_interval =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_ACTIVE_CHECK_INTERVAL") != nullptr
      ? get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_ACTIVE_CHECK_INTERVAL", 4)
      : (enable_tc_pfxt_fusion ? 4 : 1);
  const int tc_pfxt_discover_blocks =
    std::getenv("GPUCPG_ADAPTIVE_PFXT_DISCOVER_BLOCKS") != nullptr
      ? get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_DISCOVER_BLOCKS", 128)
      : (enable_tc_pfxt_fusion ? 256 : 128);

  // prepare short and long pile
  // Candidate storage is shared infrastructure, not an adaptive-only
  // optimization. Keep the old adaptive variable as a compatibility alias,
  // while the generic spelling enables the same arena for legacy GPG.
  const bool enable_tc_pfxt_candidate_arena =
    std::getenv("GPUCPG_PFXT_CANDIDATE_ARENA") != nullptr
    || (enable_tc_pfxt
      && std::getenv("GPUCPG_ADAPTIVE_PFXT_CANDIDATE_ARENA") != nullptr);
  ...
  if (enable_tc_pfxt_candidate_arena) {
    ...
    const int requested_slots = get_env_int_or_default(
      "GPUCPG_PFXT_CANDIDATE_ARENA_SLOTS",
      get_env_int_or_default(
        "GPUCPG_ADAPTIVE_PFXT_CANDIDATE_ARENA_SLOTS",
        get_env_int_or_default(
          "GPUCPG_ADAPTIVE_PFXT_SOURCE_LOCAL_MAX_SLOTS", 400000000)));
    const int short_percent = get_env_int_or_default(
      "GPUCPG_PFXT_CANDIDATE_ARENA_SHORT_PERCENT",
      get_env_int_or_default(
        "GPUCPG_ADAPTIVE_PFXT_CANDIDATE_ARENA_SHORT_PERCENT", 25));
    const int memory_percent = get_env_int_or_default(
      "GPUCPG_PFXT_CANDIDATE_ARENA_MEMORY_PERCENT",
      get_env_int_or_default(
        "GPUCPG_ADAPTIVE_PFXT_CANDIDATE_ARENA_MEMORY_PERCENT", 70));
    ...
    if (!capacities.valid) {
      throw std::runtime_error(
        "pfxt candidate arena cannot satisfy minimum one-shot capacity");
    }
  }
  TcPfxtNodePile short_pile(
    _h_pfxt_nodes,
  ...
  if (enable_tc_pfxt) {
    ...
    std::cout << "adaptive_pfxt_short_pile_reserved=" << short_capacity << '\n';
    std::cout << "adaptive_pfxt_max_pairs=" << tc_pfxt_max_pairs << '\n';
    std::cout << "adaptive_pfxt_requested_execution_path="
      << (enable_tc_pfxt_source_local_candidate
        ? "source_local_cuda (BVSS discovery may be bypassed)"
        : "bvss_tensor_core") << '\n';
    std::cout << "adaptive_pfxt_fusion=" << (enable_tc_pfxt_fusion ? 1 : 0)
      ...
      << ", classify_validate=" << (enable_tc_pfxt_classify_validate ? 1 : 0)
        ...
        << ", ordered_frontier="
        << (std::getenv("GPUCPG_ADAPTIVE_PFXT_ORDERED_FRONTIER") != nullptr ? 1 : 0)
        << ", ordered_frontier_min_products="
        << get_env_int_or_default(
          "GPUCPG_ADAPTIVE_PFXT_ORDERED_FRONTIER_MIN_PRODUCTS", 100000)
        ...
        << get_env_int_or_default("GPUCPG_ADAPTIVE_PFXT_TILE_NATIVE_MIN_PRODUCTS", 4096)
        << ", compact_static_devs="
  ...
  if (pe_method == PfxtExpMethod::BASIC) {
    ...
    int prev_step_short_pile_size{0};
        ...
        // Production source-local traversal does not consume BVSS. The
        // diagnostic replay temporarily routes its exact aligned operand
        // through the legacy interface so scalar and MMA see identical work.
        auto& tc_pfxt_bvss =
          enable_tc_pfxt_aligned_bvss
        ...
        const bool profile_tc_pfxt_setup_stages =
          std::getenv("GPUCPG_ADAPTIVE_PFXT_SETUP_STAGE_PROFILE") != nullptr;
        const auto tc_pfxt_static_setup_begin =
          ...
          tc_pfxt_static_hit =
            ...
            && _tc_pfxt_static_cache->can_reuse_source_local_static(
              N,
              M,
              tc_pfxt_need_compact_static_devs && !tc_pfxt_cached_ordinary_safe);
          if (tc_pfxt_static_hit) {
            std::cout << "adaptive_pfxt_static_cache=hit tc_static=1\n";
          }
          else {
          if ((enable_tc_pfxt_compact_static_devs || enable_tc_pfxt_aligned_bvss)
            ...
            const bool gpu_compact_setup =
              ...
              && !enable_tc_pfxt_aligned_bvss
              && std::getenv("GPUCPG_ADAPTIVE_PFXT_CPU_COMPACT_SETUP") == nullptr;
            if (gpu_compact_setup) {
              TcPfxtCompactBuildStageStats compact_stage_stats;
              const int compact_edges =
                build_tc_pfxt_compact_deviation_offsets_on_device(
                N, d_fanout_adjp, d_fanout_adjncy,
                d_succs, d_dists_cache, tc_pfxt_compact_static_devs,
                profile_tc_pfxt_setup_stages ? &compact_stage_stats : nullptr,
                false);
              tc_pfxt_chain_product_upper_bounds.resize(N, thrust::no_init);
              compute_tc_pfxt_chain_product_upper_bounds
                <<<std::max(1, ROUNDUPBLOCKS(N, 256)), 256>>>(
                  N,
                  thrust::raw_pointer_cast(
                    tc_pfxt_compact_static_devs.offsets.data()),
                  d_succs,
                  d_tc_pfxt_next_dev_vertex,
                  thrust::raw_pointer_cast(
                    tc_pfxt_chain_product_upper_bounds.data()));
              cudaCheckErrors("source-local chain-product bound failed");
              const auto max_it = thrust::max_element(
                tc_pfxt_chain_product_upper_bounds.begin(),
                tc_pfxt_chain_product_upper_bounds.end());
              tc_pfxt_max_chain_products =
                max_it == tc_pfxt_chain_product_upper_bounds.end()
                  ? 0 : *max_it;
              const bool ordinary_safe =
                tc_pfxt_max_chain_products > 0
                && tc_pfxt_max_chain_products
                  < static_cast<unsigned long long>(tc_pfxt_ordinary_low);
              const bool experimental_lazy_raw = ordinary_safe
                && std::getenv(
                  "GPUCPG_EXPERIMENTAL_LAZY_RAW_DEVIATIONS") != nullptr;
              if (!experimental_lazy_raw) {
                build_tc_pfxt_compact_deviation_payload_on_device(
                  N, compact_edges, d_fanout_adjp, d_fanout_adjncy,
                  d_fanout_wgts, d_succs, d_dists_cache,
                  tc_pfxt_compact_static_devs,
                  profile_tc_pfxt_setup_stages ? &compact_stage_stats : nullptr);
              }
              if (profile_tc_pfxt_setup_stages) {
                std::cout << "adaptive_pfxt_static_setup_stage stage=compact_offsets"
                  << " setup_ms=" << compact_stage_stats.offsets_ms << '\n';
                std::cout << "adaptive_pfxt_static_setup_stage stage=compact_payload"
                  << " setup_ms=" << compact_stage_stats.payload_ms << '\n';
              }
              if (std::getenv("GPUCPG_ADAPTIVE_PFXT_VALIDATE_GPU_COMPACT_SETUP") != nullptr) {
                ...
                std::cout << "adaptive_pfxt_gpu_compact_equivalence"
                  << " match=" << (matches ? 1 : 0)
              ...
              std::cout << "adaptive_pfxt_compact_builder=gpu"
                << " edges=" << compact_edges << " scalar_d2h=1\n";
              if (tc_pfxt_use_static_cache) {
                _tc_pfxt_static_cache->compact_devs_valid = !experimental_lazy_raw;
                _tc_pfxt_static_cache->max_chain_products =
                  tc_pfxt_max_chain_products;
            ...
            if (enable_tc_pfxt_aligned_bvss) {
              ...
              std::cout << "adaptive_pfxt_aligned_bvss_n_vss="
            ...
            std::cout << "adaptive_pfxt_compact_static_deviation_edges="
            ...
            std::cout << "adaptive_pfxt_static_setup_stage stage=compact_deviations"
            ...
            std::cout << "adaptive_pfxt_static_deviation_edges="
              << h_static_devs.edge_ids.size() << '\n';
          }
          }
		      std::cout << "adaptive_pfxt_max_chain_substeps=" << std::max(1, graph_diameter) << '\n';
		    }
        if (enable_tc_pfxt
          ...
          if (profile_tc_pfxt_setup_stages) {
            ...
            std::cout << "adaptive_pfxt_static_setup_stage stage=chain_bounds"
        ...
        const bool tc_pfxt_ordinary_topology_safe =
          ...
          && tc_pfxt_max_chain_products
            < static_cast<unsigned long long>(tc_pfxt_ordinary_low);
        if (enable_tc_pfxt) {
          std::cout << "adaptive_pfxt_ordinary_topology_bound"
        ...
        const double tc_pfxt_static_setup_ms = enable_tc_pfxt
          ...
          : 0.0;
        if (enable_tc_pfxt) {
          std::cout << "adaptive_pfxt_static_setup"
	      ...
	      if (curr_expansion_window_size > 0) {
	        ...
	        if (enable_tc_pfxt) {
	          ...
	          if (enable_tc_pfxt_single_pass) {
                  ...
                  tc_pfxt_ordinary_topology_safe
                    && tc_pfxt_compact_static_devs.dsts.empty(),
                  thrust::raw_pointer_cast(
                    tc_pfxt_chain_product_upper_bounds.data()),
	              tc_pfxt_single_pass_fallback_long_pile,
	              curr_step,
	          ...
	          if (short_pile_size >= k) {
	            ...
	            std::cout
	              << "adaptive_pfxt_source_local_static_devs_released_at_long_pile_size="
	              << long_pile_size << '\n';
	          }
	          std::cout << "adaptive expanding...short_pile_size=" << short_pile_size
	            << " (added " << h_num_short_paths << " short paths)\n";
	          std::cout << "adaptive expanding...long_pile_size=" << long_pile_size
	            << " (added " << h_num_long_paths << " long paths)\n";
	        }
	        else {
          const bool gpg_arena_needs_guard_count =
            enable_tc_pfxt_candidate_arena
            && sizeof(PfxtNode) * static_cast<double>(long_pile_size)
              >= 0.50 * long_pile_size_limit_in_bytes;
          if (enable_tc_pfxt_candidate_arena
              && !gpg_arena_needs_guard_count) {
            const int base_short = short_pile_size;
            const int base_long = long_pile_size;
            cudaMemset(d_num_long_paths, 0, sizeof(int));
            const bool store_long = short_pile_size < k;
            expand_short_pile_arena<<<num_blks, BLOCKSIZE>>>(
              d_fanout_adjp, d_fanout_adjncy, d_fanout_wgts, d_succs,
              d_dists_cache, d_short_pile, d_long_pile, h_window_start,
              h_window_end, tail_short.get(), tail_long.get(), h_split,
              final_window_size > 0
                ? final_split : std::numeric_limits<float>::max(),
              store_long, short_pile.capacity(), long_pile.capacity(),
              d_num_long_paths);
            int h_tail_short = base_short;
            int h_tail_long = base_long;
            int h_overflow = 0;
            cudaMemcpy(&h_tail_short, tail_short.get(), sizeof(int),
              cudaMemcpyDeviceToHost);
            cudaMemcpy(&h_tail_long, tail_long.get(), sizeof(int),
              cudaMemcpyDeviceToHost);
            cudaMemcpy(&h_overflow, d_num_long_paths, sizeof(int),
              cudaMemcpyDeviceToHost);
            if (h_overflow != 0
                || h_tail_short > short_pile.capacity()
                || h_tail_long > long_pile.capacity()) {
              throw std::runtime_error(
                "gpg candidate arena capacity exceeded (one-shot run aborted)");
            }
            h_num_short_paths = h_tail_short - base_short;
            h_num_long_paths = h_tail_long - base_long;
            short_pile_size = h_tail_short;
            long_pile_size = h_tail_long;
            short_pile.commit_size(short_pile_size);
            long_pile.commit_size(long_pile_size);
            d_short_pile = short_pile.data();
            d_long_pile = long_pile.empty() ? nullptr : long_pile.data();

            // Legacy GPG used its preliminary count to trigger this pruning
            // before fill. The arena can safely perform it after fused append.
            if (sizeof(PfxtNode) * static_cast<double>(long_pile_size)
                  > long_pile_size_limit_in_bytes
                && final_window_size == 0 && short_pile_size < k) {
              const int needed = k - short_pile_size;
              if (needed <= 0 || needed > long_pile_size) {
                throw std::runtime_error(
                  "gpg arena cannot form requested final window");
              }
              thrust::sort(long_pile.begin(), long_pile.end(), pfxt_node_comp());
              PfxtNode boundary;
              cudaMemcpy(&boundary, d_long_pile + needed - 1,
                sizeof(PfxtNode), cudaMemcpyDeviceToHost);
              final_split = boundary.slack;
              final_window_size = static_cast<int>(thrust::remove_if(
                long_pile.begin(), long_pile.end(),
                [final_split] __host__ __device__ (const PfxtNode& n) {
                  return n.slack > final_split;
                }) - long_pile.begin());
              long_pile_size = final_window_size;
              long_pile.commit_size(long_pile_size);
              d_long_pile = long_pile.data();
              set_kernel<<<1, 1>>>(tail_long.get(), long_pile_size);
            }
            std::cout << "gpg arena expanding...short_pile_size="
              << short_pile_size << " (added " << h_num_short_paths
              << " short paths)\n";
            std::cout << "gpg arena expanding...long_pile_size="
              << long_pile_size << " (added " << h_num_long_paths
              << " long paths)\n";
          }
          else {
    ...
    if (enable_tc_pfxt_candidate_arena) {
      std::cout << "adaptive_pfxt_candidate_arena_summary"
    ...
    if (enable_tc_pfxt) {
      ...
      std::cout << "runtime_summary_adaptive_breakdown"
        ...
        << '\n';
      std::cout << "runtime_summary_adaptive_stages"
        ...
        << " execution_path=source_local_cuda"
      ...
      const auto conceptual_breakdown =
        tc_pfxt::conceptual_runtime_stage_breakdown(
          tc_pfxt::RawRuntimeStageMs{
            ...
            pfxt_summary_candidate_pair_meta_ms,
            pfxt_summary_candidate_prepare_ms});
      std::cout << "runtime_summary_adaptive_conceptual"
        << " prepare_tc_query_ms="
      ...
      if (light_tc_pfxt_stage_profile) {
        std::cout << "runtime_summary_adaptive_candidate_detail"
      ...
      std::cout << "runtime_summary_adaptive_candidate_volume"
      ...
      if (pfxt_summary_fused_shadow_pairs > 0
        ...
        std::cout << "runtime_summary_adaptive_fused_interface_shadow"
      ...
      if (pfxt_summary_in_discovery_substeps > 0
        ...
        std::cout << "runtime_summary_adaptive_in_discovery_short_only"
      ...
      if (pfxt_summary_direct_pair_meta_substeps > 0
        ...
        std::cout << "runtime_summary_adaptive_direct_pair_meta"
	      ...
	      if (pfxt_summary_source_local_materialization_substeps > 0
	        ...
	        std::cout << "runtime_summary_adaptive_spur_source_grouped"
	        ...
	        if (pfxt_summary_source_local_filter_tiles > 0) {
	          std::cout << "runtime_summary_adaptive_tile_filter"
	        ...
	        if (pfxt_summary_source_local_bound_tiles > 0) {
	          std::cout << "runtime_summary_adaptive_short_tile_bounds"
	        ...
	        if (pfxt_summary_tile_handoff_tiles > 0
	          ...
	          std::cout << "runtime_summary_adaptive_tile_handoff_fusion"
	        ...
	        if (pfxt_summary_tile_resident_shadow_tiles > 0) {
	          ...
	          const std::uint64_t mismatches =
	            ...
	            + pfxt_summary_tile_resident_shadow_all_long_mismatches
	            + pfxt_summary_tile_resident_shadow_all_skip_mismatches;
	          std::cout << "runtime_summary_adaptive_tile_resident_lpq_shadow"
	        ...
	        if (pfxt_summary_tile_resident_cheap_shadow_tiles > 0) {
	          const std::uint64_t estimated_bytes_avoided =
	            tc_pfxt::estimated_tile_resident_lpq_bytes_avoided(
	              ...
	              sizeof(int4) + sizeof(tc_pfxt::SourceLocalTileBounds),
	              sizeof(PfxtNode));
	          std::cout << "runtime_summary_adaptive_tile_resident_lpq_cheap_shadow"
	      ...
	      if (enable_tc_pfxt_compressed_lpq) {
        std::cout << "runtime_summary_adaptive_compressed_lpq"
    ...
    if (std::getenv("GPUCPG_ADAPTIVE_PFXT_ADAPTIVE_DEFER") != nullptr
      ...
      unsigned int telemetry_size = 0;
      cudaMemcpy(
        &telemetry_size,
        thrust::raw_pointer_cast(
          tc_pfxt_scratch.defer_oracle_telemetry_size.data()),
        sizeof(unsigned int),
        cudaMemcpyDeviceToHost);
      cudaCheckErrors("adaptive telemetry size copy failed");
      const auto recorded = std::min<std::size_t>(
        telemetry_size,
        tc_pfxt_scratch.defer_oracle_telemetry.size());
      thrust::host_vector<tc_pfxt::AdaptiveTelemetryEntry> telemetry(
        tc_pfxt_scratch.defer_oracle_telemetry.begin(),
        tc_pfxt_scratch.defer_oracle_telemetry.begin() + recorded);
      unsigned long long ordinary_steps = 0;
      unsigned long long deferred_steps = 0;
      unsigned long long switches = 0;
      auto previous = tc_pfxt::AdaptiveMode::UNINITIALIZED;
      for (const auto& entry : telemetry) {
        ordinary_steps += entry.mode == tc_pfxt::AdaptiveMode::ORDINARY;
        deferred_steps += entry.mode == tc_pfxt::AdaptiveMode::DEFERRED;
        const bool switched =
          (previous == tc_pfxt::AdaptiveMode::ORDINARY
           || previous == tc_pfxt::AdaptiveMode::DEFERRED)
          && previous != entry.mode;
        switches += switched;
        std::cout << "adaptive_mode_step"
          << " outer_step=" << entry.outer_step
          << " chain_substep=" << entry.chain_substep
          << " mode="
          << (entry.mode == tc_pfxt::AdaptiveMode::DEFERRED
                ? "deferred" : "normal")
          << " active_paths=" << entry.active_paths
