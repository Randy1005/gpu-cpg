...
struct CandidateArenaCapacities {
  int total = 0;
...
inline CandidateArenaCapacities
candidate_arena_capacities(const int requested_slots,
                           const std::uint64_t free_bytes,
                           const std::uint64_t node_bytes,
                           const int minimum_short_capacity,
                           const int short_percent = 25,
                           const int memory_percent = 70) {
  CandidateArenaCapacities result;
  if (requested_slots <= 0 || free_bytes == 0 || node_bytes == 0
      || minimum_short_capacity < 0
      || short_percent <= 0 || short_percent >= 100
      || memory_percent <= 0 || memory_percent > 100) {
    return result;
  }
  const auto memory_slots =
    (free_bytes / 100u) * static_cast<std::uint64_t>(memory_percent)
    / node_bytes;
  const auto bounded = std::min<std::uint64_t>(
    static_cast<std::uint64_t>(requested_slots), memory_slots);
  result.total = static_cast<int>(std::min<std::uint64_t>(
    bounded, static_cast<std::uint64_t>(INT_MAX)));
  result.short_capacity = static_cast<int>(
    static_cast<std::int64_t>(result.total) * short_percent / 100);
  result.long_capacity = result.total - result.short_capacity;
  result.valid = result.short_capacity >= minimum_short_capacity
    && result.long_capacity > 0;
  return result;
}
