...
repo_dir=${1:-$(pwd)}
out_dir=${2:-$repo_dir/experiments/transparent_adaptive_20260825}
...
mkdir -p "$out_dir/validation" "$out_dir/timing" "$out_dir/timing-profile"
...
# Run a separate instrumented adaptive arm. Production timing remains free of
# CUDA-event profiling overhead; this arm decomposes decision and core time.
for case_name in "${cases[@]}"; do
  benchmark=$(benchmark_for "$case_name")
  log="$out_dir/timing-profile/${case_name}.adaptive.log"
  if ! timed_log "$log"; then
    wait_for_idle_gpu
    GPUCPG_TC_PFXT_LIGHT_STAGE_PROFILE=1 "$timing_bin" \
      --benchmark "$benchmark" --k 1000000 --mode adaptive \
      --warmup 1 --trials 3 >"$log" 2>&1
    timed_log "$log"
  fi
done
